const { Op } = require("sequelize");
const sequelize = require("../config/database");
const { Trip, Route, BusDetails } = require("../models");
const ApiError = require("../utils/ApiError");

const durationSQL = `CONCAT(
  FLOOR(MOD(TIME_TO_SEC(arrivalTime) - TIME_TO_SEC(departureTime) + 86400, 86400) / 3600),
  'h ',
  LPAD(
    FLOOR(MOD(MOD(TIME_TO_SEC(arrivalTime) - TIME_TO_SEC(departureTime) + 86400, 86400), 3600) / 60),
    2, '0'
  ),
  'm'
)`;

const includeRelations = [
  { model: Route,      as: "route", attributes: ["id", "routeName", "from", "to"] },
  { model: BusDetails, as: "bus",   attributes: ["id", "registrationNumber", "busType"] },
];

const withDuration = {
  attributes: {
    include: [[sequelize.literal(durationSQL), "duration"]],
  },
};

// ─── getAll ───────────────────────────────────────────────────────────────────
const getAll = async ({ page = 1, limit = 20, search, id, status, direction, routeId, day } = {}) => {
  const parsedPage  = Math.max(1, parseInt(page));
  const parsedLimit = Math.min(100, Math.max(1, parseInt(limit)));
  const offset      = (parsedPage - 1) * parsedLimit;

  // ── ID lookup: two-step to avoid JOIN ambiguity ────────────────────────────
  if (id) {
    const parsedId = parseInt(id, 10);
    if (isNaN(parsedId)) {
      return { total: 0, page: parsedPage, totalPages: 0, trips: [] };
    }

    const exists = await Trip.findByPk(parsedId);
    if (!exists) {
      return { total: 0, page: parsedPage, totalPages: 0, trips: [] };
    }

    const trip = await Trip.findOne({
      where:    { id: parsedId },
      ...withDuration,
      include:  includeRelations,
      subQuery: true,
    });

    return {
      total:      1,
      page:       parsedPage,
      totalPages: 1,
      trips:      trip ? [trip] : [],
    };
  }

  // ── All other filters ──────────────────────────────────────────────────────
  const where = {};

  if (status === "active")    where.isActive = true;
  if (status === "cancelled") where.isActive = false;
  if (direction) where.direction = direction;
  if (routeId)   where.routeId   = parseInt(routeId);
  if (day)       where.days      = { [Op.like]: `%${day}%` };

  if (search) {
    const term = `%${search.trim()}%`;

    const matchingRoutes = await Route.findAll({
      where: {
        [Op.or]: [
          { routeName: { [Op.like]: term } },
          { from:      { [Op.like]: term } },
          { to:        { [Op.like]: term } },
        ],
      },
      attributes: ["id"],
    });
    const matchingRouteIds = matchingRoutes.map((r) => r.id);

    const matchingBuses = await BusDetails.findAll({
      where: { registrationNumber: { [Op.like]: term } },
      attributes: ["id"],
    });
    const matchingBusIds = matchingBuses.map((b) => b.id);

    const orClauses = [];
    if (matchingRouteIds.length) orClauses.push({ routeId: { [Op.in]: matchingRouteIds } });
    if (matchingBusIds.length)   orClauses.push({ busId:   { [Op.in]: matchingBusIds   } });

    if (orClauses.length === 0) {
      return { total: 0, page: parsedPage, totalPages: 0, trips: [] };
    }

    where[Op.or] = orClauses;
  }

  const { count, rows } = await Trip.findAndCountAll({
    where,
    ...withDuration,
    include:  includeRelations,
    limit:    parsedLimit,
    offset,
    order:    [["createdAt", "DESC"]],
    subQuery: false,
  });

  return {
    total:      count,
    page:       parsedPage,
    totalPages: Math.ceil(count / parsedLimit),
    trips:      rows,
  };
};

// ─── getById ──────────────────────────────────────────────────────────────────
const getById = async (id) => {
  const trip = await Trip.findByPk(id, {
    ...withDuration,
    include: includeRelations,
  });
  if (!trip) throw new ApiError(404, "Trip not found");
  return trip;
};

// ─── create ───────────────────────────────────────────────────────────────────
const create = async (data) => {
  const trip = await Trip.create({
    routeId:       data.routeId,
    busId:         data.busId,
    direction:     data.direction     ?? "forward",
    departureTime: data.departureTime,
    arrivalTime:   data.arrivalTime,
    days:          data.days          ?? [],
    isActive:      true,
  });
  return getById(trip.id);
};

// ─── update ───────────────────────────────────────────────────────────────────
const update = async (id, data) => {
  const trip = await Trip.findByPk(id);
  if (!trip) throw new ApiError(404, "Trip not found");

  await trip.update({
    routeId:       data.routeId       ?? trip.routeId,
    busId:         data.busId         ?? trip.busId,
    direction:     data.direction     ?? trip.direction,
    departureTime: data.departureTime ?? trip.departureTime,
    arrivalTime:   data.arrivalTime   ?? trip.arrivalTime,
    days:          data.days          ?? trip.days,
    status:        data.status        ?? trip.status,
    isActive:      data.isActive      !== undefined ? data.isActive : trip.isActive,
  });

  return getById(trip.id);
};

// ─── toggleActive ─────────────────────────────────────────────────────────────
const toggleActive = async (id) => {
  const trip = await Trip.findByPk(id);
  if (!trip) throw new ApiError(404, "Trip not found");
  await trip.update({ isActive: !trip.isActive });
  return getById(trip.id);
};

// ─── remove ───────────────────────────────────────────────────────────────────
const remove = async (id) => {
  const trip = await Trip.findByPk(id);
  if (!trip) throw new ApiError(404, "Trip not found");
  await trip.destroy();
  return { message: "Trip deleted successfully" };
};

// ─── getStats ─────────────────────────────────────────────────────────────────
const getStats = async () => {
  const [total, active, cancelled] = await Promise.all([
    Trip.count(),
    Trip.count({ where: { isActive: true  } }),
    Trip.count({ where: { isActive: false } }),
  ]);

  return { total, active, cancelled };
};

module.exports = { getAll, getById, create, update, toggleActive, remove, getStats };